...
#     has_run = True   # prevent re-entry
#     imu = GolfIMU()
#     imu.connect()

#     fs = 400  # Hz
#     num_samples = 100
#     accel_data, gyro_data, t_data = [], [], []
...
#             t_data.append(t)
#             # print(f"{t:.3f}s | Acc: {accel} | Gyro: {gyro}")

#     imu.close()
#     print(f"Collected {len(accel_data)} samples.")
...
#     # 🔹 Low-pass filter (keep smooth motion)
#     b, a = signal.butter(3, 20 / (fs / 2), btype='low')
#     ax_filt = signal.filtfilt(b, a, accel_arr[:, 0])
#     ay_filt = signal.filtfilt(b, a, accel_arr[:, 1])
#     az_filt = signal.filtfilt(b, a, accel_arr[:, 2])
#     gx_filt = signal.filtfilt(b, a, gyro_arr[:, 0])
#     gy_filt = signal.filtfilt(b, a, gyro_arr[:, 1])
#     gz_filt = signal.filtfilt(b, a, gyro_arr[:, 2])

#     # # High-pass filter added (remove drift / very low frequency bias)
#     # bh, ah = signal.butter(1, 0.01 / (fs / 2), btype='high')
#     # ax_filt = signal.filtfilt(bh, ah, ax_filt)
#     # ay_filt = signal.filtfilt(bh, ah, ay_filt)
#     # az_filt = signal.filtfilt(bh, ah, az_filt)
...
#     # 🔹 New: Save filtered acceleration
...
class GolfIMU:
    ...
    def read_data(self):
        ...
        file_content = ""
        while True:
            print("2: Reading line")
            line = self.ser.readline().decode("utf-8", errors="ignore").strip()
            print("3: Received line:", line)
            if line == "EOF":
                break
            if line.startswith("I (") or line.startswith("E ("):
                continue
        ...
        print("4: Finished reading, raw file content:\n", file_content)
        lines = file_content.strip().split("\n")
